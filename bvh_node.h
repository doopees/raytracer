...
{
public:
    // We modify the hittable_list constructor to convert unique_ptr to shared_ptr
    bvh_node(hittable_list list)
    {
        std::vector<std::shared_ptr<hittable>> shared_objects;
        for (auto &obj : list.objects)
        {
            shared_objects.push_back(std::shared_ptr<hittable>(std::move(obj)));
        }
        *this = bvh_node(shared_objects, 0, shared_objects.size());
    }

    bvh_node(std::vector<std::shared_ptr<hittable>> &objects, size_t start, size_t end)
    {
        // Build the bounding box of the span of objects
        bbox = aabb::empty; // You might need to define aabb::empty in aabb.h
        for (size_t i = start; i < end; i++)
            bbox = aabb(bbox, objects[i]->bounding_box());

        int axis = bbox.longest_axis(); // Get the best axis to split on
